...
(function (js, ajax, paginatedListService) {
    ...
    var employeeSkillsList = paginatedListService.create('skills', null, {
        elementDrawer: function elementDrawer(item, state) {
            var html = '<li class="list-group-item"><a class="reset" href="/skills/details?id=' + item.Id + '">' + item.Name + '</a></li>';
            if (!state.viewDetails.readOnly) {
                html = '<li class="list-group-item"><span class="remove-skill" data-skill-id="' + item.Id + '"><i class="fa fa-times text-danger"></i> ' + item.Name + '</span></li>';
            }
            return html;
        },
    ...
    function addSkillsFetcher(state) {
        ...
        return skillsPromise.then(function (listResults) {
    ...
    var addSkillsList = paginatedListService.create('add-skills', addSkillsFetcher, {
        elementDrawer: function elementDrawer(skill, state) {
            return '<li class="list-group-item"><span class="add-skill" data-skill-id="' + skill.Id + '"><i class="fa fa-plus text-success"></i> ' + skill.Name + '</span></li>';
        },
