...
(function (js, ajax, paginatedListService) {
    ...
    var skillEmployeesList = paginatedListService.create('employees', null, {
        elementDrawer: function elementDrawer(item, state) {
            var html = '<li class="list-group-item"><a class="reset" href="/employees/details?id=' + item.Id + '">' + item.Name + '</a></li>';
            if (!state.viewDetails.readOnly) {
                html = '<li class="list-group-item"><span class="remove-employee" data-employee-id="' + item.Id + '"><i class="fa fa-times text-danger"></i> ' + item.Name + '</span></li>';
            }
            return html;
        },
    ...
    function addEmployeesFetcher(state) {
        ...
        return employeesPromise.then(function (listResults) {
    ...
    var addEmployeesList = paginatedListService.create('add-employees', addEmployeesFetcher, {
        elementDrawer: function elementDrawer(employee, state) {
            return '<li class="list-group-item"><span class="add-employee" data-employee-id="' + employee.Id + '"><i class="fa fa-plus text-success"></i> ' + employee.Name + '</span></li>';
        },
    ...
    window.skillDetails.elements = {
        ...
        addEmployeesList: addEmployeesList,
        htmlNodes: htmlNodes
    };
